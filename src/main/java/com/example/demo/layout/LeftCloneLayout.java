 ...
 * 根节点在右侧，所有子节点在左侧
 */
public class LeftCloneLayout extends CloneBasedLayout {
    ...
    protected void doLayout(MindMap mindMap, Map<String, NodeInfo> nodeInfoMap, double canvasWidth, double canvasHeight) {
        ...
        // 将根节点放在画布右侧，除非它已经被手动定位
        if (!rootNode.isManuallyPositioned()) {
            rootNode.setX(canvasWidth * 0.8);
        ...
        // 计算左侧X坐标
        double x = rootNode.getX() - HORIZONTAL_GAP - rootNode.getWidth();
        ...
        for (MindMapNode child : children) {
            ...
            if (!child.isManuallyPositioned()) {
                ...
                // 递归布局子节点的子节点
                layoutChildrenLeft(child, x - child.getWidth() - HORIZONTAL_GAP);
            }
